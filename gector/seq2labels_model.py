...
class Seq2Labels(Model):
    ...
    def forward(self,  # type: ignore
        ...
        if labels is not None and d_tags is not None:
            ...
            loss_labels = sequence_cross_entropy_with_logits(logits_labels, labels, mask * weights, average='token',
                                                             label_smoothing=self.label_smoothing)
            loss_d = sequence_cross_entropy_with_logits(logits_d, d_tags, mask * weights, average='token')
...
@Model.register("seq2labelsstudent")
class Seq2LabelsStudent(Model):
    """
    This ``Seq2Labels`` simply encodes a sequence of text with a stacked ``Seq2SeqEncoder``, then
    predicts a tag (or couple tags) for each token in the sequence.
    ...
    def __init__(self, vocab: Vocabulary,
                 ...
                 teacher: Seq2Labels=None) -> None:
        super(Seq2LabelsStudent, self).__init__(vocab, regularizer)
    ...
    def forward(self,  # type: ignore
        ...
        if labels is not None and d_tags is not None:
            ...
            with torch.no_grad():
                teacher_output = self.teacher(tokens=tokens)
            kl_loss = nn.KLDivLoss(reduction="batchmean", log_target=True)

            kl_labels = kl_loss(F.log_softmax(logits_labels.view(batch_size * sequence_length, -1), dim=-1), F.log_softmax(teacher_output['logits_labels'].view(batch_size * sequence_length, -1).detach(),dim=-1))
            kl_d = kl_loss(F.log_softmax(logits_d.view(batch_size * sequence_length, -1), dim=-1), F.log_softmax(teacher_output['logits_d_tags'].view(batch_size * sequence_length, -1).detach(),dim=-1))

            output_dict["loss"] = loss_labels + loss_d + (kl_labels + kl_d) 
